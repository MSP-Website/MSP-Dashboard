...
async function getDataGalleryEvents(pageNum) {
  ...
  document.querySelector("#Gallery .container-images").innerHTML = `
          ...
          `;
  for (let i = 0; i < data["results"].length; i++) {
    if (data["results"][i]["name"] === "Events") {
      document.querySelector("#Gallery .container-images").innerHTML += `
              <div class="p-con-img">
              <div class="conatiner-img">
               <span class="zoom-icon"><img src="../assests/zoom-in.webp" alt=""></span>
                        <span class="edit-icon"><img src="../assests/edit.webp" alt=""></span>
                        
              <span onclick="DeleteFromGallery(${
                data["results"][i]["_id"]
              });getDataGalleryAll()">Delete</span>
              <img src="http://164.92.244.59:3000/${data["results"][i][
                "image"
              ].slice(27)}" alt="">
              </div>
              </div>
...
async function getDataGallerySessions(pageNum) {
  ...
  if ("next" in data) {
    ...
    btnNex.onclick = () => {
      pageNum = data["next"]["page"];
      getDataGallerySessions(pageNum);
    };
  } else {
    btnNex.style.display = "none";
  }
  ...
  document.querySelector("#Gallery .container-images").innerHTML = `
          ...
          `;
  for (let i = 0; i < data["results"].length; i++) {
    if (data["results"][i]["name"] == "Sessions") {
      document.querySelector("#Gallery .container-images").innerHTML += `
              <div class="p-con-img">
              <div class="conatiner-img">
               <span class="zoom-icon"><img src="../assests/zoom-in.webp" alt=""></span>
...
function postGallery() {
  ...
  // formdata.append("type", sBtn_text.innerText);
  formdata.append(
    "image",
...
function editingImage(imageId) {
  ...
  document.querySelector("#addOrEdit").innerHTML = "Edit";
